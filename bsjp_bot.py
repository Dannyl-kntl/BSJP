# ===== BSJP SCAN SORE - REVISI FINAL (DOWNLOAD MASSAL + DATA FRESH + BERITA) =====
# Fitur: Download massal (stabil), Data Fresh, Sentimen Google, IDX, Foreign Flow
...
# DAFTAR SAHAM IDX (Top 15 likuid) + Tambahan
LIST_SAHAM = [
...
def cek_data_fresh(df):
    ...
    try:
        ...
        if last_date == today_date:
            ...
            return f"⚠️ DATA TELAT! (H-1) - Data: {last_date}", f"⚠️ PERINGATAN: Harga yang dipakai adalah harga {last_date}, BUKAN hari ini!"
    except:
...
def scrape_berita_google(kode):
    ...
    try:
        ...
        if resp.status_code == 200:
            ...
            pos = ['naik', 'melesat', 'rekor', 'laba', 'dividen', 'ekspansi', 'akuisisi', 'kerjasama']
...
def analisis_bsjp(kode, df_saham=None):
    """
    Analisis BSJP dengan data dari yf.download (massal)
    """
    try:
        # Jika df_saham None, ambil sendiri (fallback)
        if df_saham is None:
            saham = yf.Ticker(kode)
            df = saham.history(period="120d")
        else:
            # Data sudah di-download massal, ambil per saham
            if kode not in df_saham or df_saham[kode].empty:
                return None
            df = df_saham[kode].copy()
...
def main():
    ...
    print(f"🚀 BSJP SCAN SORE (FINAL) - {waktu}")
    ...
    try:
        ...
        print("✅ Download massal selesai!")
    except Exception as e:
        print(f"❌ Gagal download massal: {e}")
        print("🔄 Fallback ke download satu-satu...")
        data = None
    
    semua_hasil = []
    for kode in LIST_SAHAM:
        if data is not None:
            res = analisis_bsjp(kode, data)
        else:
            # Fallback: download satu-satu
            res = analisis_bsjp(kode, None)
        if res:
            semua_hasil.append(res)
        # Jeda sebentar biar tidak kena blokir
        time.sleep(0.3)
    ...
    pesan = f"<b>📊 BSJP SCAN SORE (FINAL)</b> - {waktu}\n"
    ...
    if sinyal_mati:
        ...
        for h in sorted(sinyal_mati, key=lambda x: x['rsi'], reverse=True)[:3]:
